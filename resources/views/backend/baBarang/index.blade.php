                ...
                <div class="card-body">
                    @if(Auth::user()->role == 'desa')
                        <button type="button" class="btn btn-sm btn-blue waves-effect waves-light float-end" id="createData" onclick="createData()">
                            <i class="mdi mdi-plus-circle"></i> Berita Acara Serah Terima Barang Baru
                        </button>
                    @endif
...
@include('backend.baBarang._form')
@endsection
@push('scripts')
<script src="{{ asset('template/barangjasa/admin/ba-barang.js') }}?{{ date('ymdshi') }}"></script>
@if($errors->any())
